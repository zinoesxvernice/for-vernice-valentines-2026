...
document.getElementById("forVernice").addEventListener("click",()=>{
  showPanel(1);
  if(!musicPlayed){music.play().catch(()=>{}); musicPlayed=true;}
});
...
function positionTimelineEventsAndDrawLine(){
  ...
  const minTop=20, maxTop=svgRect.height-40;
  const amplitude=Math.min(40,(maxTop-minTop)/2);
...
function startDrag(x,y){const rect=modalWindow.getBoundingClientRect(); offsetX=x-rect.left; offsetY=y-rect.top; isDragging=true; modalWindow.style.transition="none";}
function dragMove(x,y){if(!isDragging) return; modalWindow.style.left=x-offsetX+"px"; modalWindow.style.top=y-offsetY+"px"; modalWindow.style.position="fixed";}
function stopDrag(){if(isDragging) isDragging=false; modalWindow.style.transition="all 0.3s ease";}
